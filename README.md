...
Notes: Git LFS, dataset size and next steps
----------------------------------------

- Large binary files (image folders and model pickles) are tracked with Git LFS. After cloning:
...
- GitHub LFS may incur storage/bandwidth limits. For easier public distribution of the full dataset consider uploading images to AWS S3 and putting links in this README. For models only, GitHub Releases are a practical option.

Suggested follow-ups I can do for you
------------------------------------

1) Create `requirements.txt` and commit it.
2) Create a GitHub Release and attach model files in `models/` for simple downloads.
3) Upload dataset to S3 and add presigned links to this README.
4) Add a small `sample_data/` set (10–20 images) for quick demos without LFS.

If you want me to commit and push this README to `origin/main`, tell me now and I'll run the git commands and push.
